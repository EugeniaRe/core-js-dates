...
function formatDate(date) {
  ...
  const fullDate = `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}`;
  let hours = `${newDate.getUTCHours()}`;
  if (newDate.getUTCHours() > 12) {
    hours = `${newDate.getUTCHours() % 12}`;
  }
  let end = 'AM';
  if (newDate.getUTCHours() > 11) {
    end = 'PM';
  }
  ...
  if (newDate.getSeconds() < 10) {
    sec = `0${newDate.getSeconds()}`;
  }
  const fullTime = `${hours}:${min}:${sec} ${end}`;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (start.getTime() <= end.getTime()) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      ...
      resultArr.push(workDay);
      const startDay = start.getDate();
      start.setDate(startDay + 1);
      if (start.getTime() > end.getTime()) break;
    }
    start.setDate(start.getDate() + countOffDays);
  }
  const resultStrArr = resultArr.map((date) => {
    const day =
